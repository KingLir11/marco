...
const ChatBot = () => {
  ...
  const [messages, setMessages] = useState([{
    type: "bot",
    text: "Ask me anything about your trip 🌤️"
  }]);
  ...
  return <div className="fixed bottom-4 right-4 z-50">
      {isOpen && <div className="bg-white rounded-lg shadow-xl w-80 sm:w-96 mb-4 overflow-hidden border">
          <div className="bg-primary p-4 text-white">
            <h3 className="font-medium">Marco Assistant</h3>
          </div>
          
          <div className="h-80 overflow-y-auto p-4">
            {messages.map((message, index) => <div key={index} className={`mb-3 ${message.type === "user" ? "text-right" : "text-left"}`}>
                <div className={`inline-block rounded-lg px-4 py-2 max-w-[80%] ${message.type === "user" ? "bg-primary text-white" : "bg-gray-100 text-gray-800"}`}>
                  {message.text}
                </div>
              </div>)}
          </div>
          ...
          <div className="border-t p-4 flex">
            <Input placeholder="Type your question..." value={input} onChange={e => setInput(e.target.value)} onKeyPress={handleKeyPress} className="flex-1 mr-2" />
      ...
      <Button onClick={() => setIsOpen(!isOpen)} className="bg-primary hover:bg-primary/90 h-12 w-12 rounded-full shadow-lg">
